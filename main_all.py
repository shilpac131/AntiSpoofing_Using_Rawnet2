...
import yaml
from data_utils import genSpoof_list,genSpoof_list_ASVspoof21,Dataset_ASVspoof2019_train,Dataset_ASVspoof2021_eval,Dataset_ASVspoof2019_eval
...
def evaluate_accuracy(dev_loader, model, device):
    ...
    for batch_x, batch_y in dev_loader:
        ...
        if (batch_pred == batch_y).item() and batch_pred.item() == 0:
            count_spoof = count_spoof + 1
        elif (batch_pred == batch_y).item() and batch_pred.item() == 1:
            count_bonafide = count_bonafide + 1
        if (batch_y.item() == 0):
            spoof  = spoof + 1
        elif(batch_y.item() == 1):
            bonafide = bonafide + 1
    return 100 * (num_correct / num_total), count_spoof, count_bonafide, spoof, bonafide
...
if __name__ == '__main__':
    ...
    # Dataset
    parser.add_argument('--database_path', type=str, default='/home/shilpa/datasets/ASVspoof2019/LA/ASVspoof2019_LA_eval/flac/', help='Change this to user\'s full directory address of DF database. We assume that all three ASVspoof 2019 LA train, LA dev and ASVspoof2021 DF eval data folders are in the same database_path directory.')
    '''
    % database_path/
    %   |- DF
    %      |- ASVspoof2021_DF_eval/flac /home/shilpa/datasets/ASVspoof2021_DF_eval_part00/ASVspoof2021_DF_eval/flac/
    %      |- ASVspoof2019_LA_train/flac
    %      |- ASVspoof2019_LA_dev/flac /home/shilpa/datasets/ASVspoof2019/LA/ASVspoof2019_LA_eval/flac/
    '''

    parser.add_argument('--protocols_path', type=str, default='/home/shilpa/datasets/mix_spoof_bondafide/keys_LA_Eval_ASVspoof19.txt', help='Change with path to user\'s DF ASVspoof2021 database protocols directory address')
    '''
    % protocols_path/
    ...
    # # Using 2019 LA Eval OR 2021 DF eval data

    if (args.database_path == '/home/shilpa/datasets/ASVspoof2021_DF_eval_part00/ASVspoof2021_DF_eval/flac/'):
        args.protocols_path = '/home/shilpa/datasets/mix_spoof_bondafide/keys_ASVspoof21.txt'
        d_label_dev,file_dev = genSpoof_list_ASVspoof21( dir_meta =  os.path.join(args.protocols_path),is_train=False,is_eval=False)
        print("For ASV spoof 2021 DF database")

    elif (args.database_path == '/home/shilpa/datasets/ASVspoof2019/LA/ASVspoof2019_LA_eval/flac/'):
        d_label_dev,file_dev = genSpoof_list( dir_meta =  os.path.join(args.protocols_path),is_train=False,is_eval=False)

    print('no. of validation trials',len(file_dev))
    dev_set = Dataset_ASVspoof2019_eval(list_IDs = file_dev, labels = d_label_dev,base_dir = os.path.join(args.database_path))
    dev_loader = DataLoader(dev_set, batch_size=1, shuffle=False)
    del dev_set,d_label_dev
    valid_accuracy, count_spoof, count_bonafide, spoof, bonafide = evaluate_accuracy(dev_loader, model, device)
    print("Accuracy is:> ", valid_accuracy)
    print(f"Correct spoof count:> {count_spoof} and correct bonafide count:> {count_bonafide}")
    print(f"Total spoof count:> {spoof} and Total bonafide count:> {bonafide}")
